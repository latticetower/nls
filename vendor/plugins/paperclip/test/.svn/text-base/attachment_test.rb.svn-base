  ...
  context "An attachment" do
    setup do
      ...
      Paperclip::Attachment.default_options.merge!({
        :path => ":rails_root***REMOVED***/:attachment/:class/:style/:id/:basename.:extension"
      })
      FileUtils.rm_rf("tmp")
      rebuild_model
      @instance = Dummy.new
    ...
    context "with a file assigned in the database" do
      ...
      should "return the proper path when filename has a single .'s" do
        assert_equal File.expand_path("./test/..***REMOVED***/avatars/dummies/original/#{@instance.id}/5k.png"), File.expand_path(@attachment.path)
      end

      should "return the proper path when filename has multiple .'s" do
        @attachment.stubs(:instance_read).with(:file_name).returns("5k.old.png")
        assert_equal File.expand_path("./test/..***REMOVED***/avatars/dummies/original/#{@instance.id}/5k.old.png"), File.expand_path(@attachment.path)
      end
