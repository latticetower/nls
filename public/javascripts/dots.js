$(function () {
    ...
    var width = 1000,
        ...
        txt = {"font": '10px Fontin-Sans, Arial', stroke: "none", fill: "#838383"},
    ...
    for (var i = 0, ii = axisx.length; i < ii; i++) {
        r.text(leftgutter + X * (i + .5), 594, axisx[i]).attr(txt);
    }
    ...
    for (var i = 0, ii = axisy.length; i < ii; i++) {
        for (var j = 0, jj = axisx.length; j < jj; j++) {
            var R = data[o] && Math.min(Math.round(Math.sqrt(data[o]/60 / Math.PI) * 4), max);
            if (R) {
                (function (dx, dy, R, value) {
                    var color = "hsb(" + [(1 - R / max) * .5, 1, .75] + ")";
                    var dt = r.circle(dx + 60 + R, dy + 10, R).attr({stroke: "none", fill: color});
                    if (R < 6) {
                        var bg = r.circle(dx + 60 + R, dy + 10, 6).attr({stroke: "none", fill: "#ffffff", opacity: .2}).hide();
                    }
					var h = Math.floor(data[o]/60)
					var m = data[o] - h*60
                    var lbl = r.text(dx + 60 + R, dy + 10, '' + h + ':' + m)
                            .attr({"font": '10px Fontin-Sans, Arial', stroke: "none", fill: "#000"}).hide();
                    var dot = r.circle(dx + 60 + R, dy + 10, max).attr({stroke: "none", fill: "#ffffff", opacity: 0});
